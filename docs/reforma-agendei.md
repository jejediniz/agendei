...
| 2026-07-29 | 8 | **Buffer entre atendimentos + granularidade de slot**: `Service.bufferMin` (default 0, teto 3h) reserva minutos extras após o atendimento antes do próximo poder começar; `Organization.slotIntervalMin` (15/30/60min, default 30) torna a grade de horários da agenda pública configurável em Configurações › Negócio. `generateAvailableSlots` e a checagem de conflito de `create-appointment.ts` passam a considerar o buffer do serviço de cada agendamento existente (via join com `Service`); a janela de busca de conflito foi alargada pelo teto de buffer para não esconder colisões geradas pelo buffer. Novos campos em `service-form.tsx` (intervalo pós-atendimento) e `organization-form.tsx` (granularidade). 2 novos testes em `appointments.test.ts` (16 no total). Validado manualmente ponta a ponta (buffer bloqueando e liberando no minuto certo; granularidade 15/30/60 gerando grades diferentes). Build/lint/typecheck/testes verdes. | feat |
| 2026-07-29 | 9 | **Remarcação self-service em 1 passo**: nova action `rescheduleCustomerAppointment` (`customer-booking.ts`) e `rescheduleAppointmentForOrganization` (`reschedule-appointment.ts`) atualizam a mesma linha do agendamento (sem cancelar+criar), reaproveitando as checagens de disponibilidade/conflito com buffer extraídas para `isWithinAvailability`/`findBufferedConflict` em `appointments.ts` (também usadas por `create-appointment.ts`, eliminando duplicação). UI: botão "Remarcar" em `customer-appointments-list.tsx` abre `RescheduleAppointmentDialog`, que reaproveita `AgendaWeekStrip` + `ProfessionalSlotTimeGrid` do fluxo público só para escolher nova data/hora (mesmo profissional/serviço). Mesma regra de elegibilidade do cancelamento (status + horário futuro). Validado manualmente ponta a ponta (mesma linha atualizada, sem duplicar). Build/lint/typecheck/testes verdes. | feat |
| 2026-07-29 | 10 | **Tema escuro**: `next-themes` (`ThemeProvider attribute="class"` em `layout.tsx`) + `@custom-variant dark` no Tailwind 4 (`globals.css`), com bloco `.dark` espelhando os 16 tokens de `:root` (primária/acento aclarados para contraste sobre fundo escuro). Toggle no `app-header.tsx`, usando `useSyncExternalStore` para detectar montagem no cliente sem violar a regra do React Compiler que proíbe `setState` dentro de `useEffect`. Varredura dos ~17 arquivos com cor fixa identificados na Fase 1 (mensagens de erro `text-rose-600`, variants de `badge`/`button`, cores de status em `appointment-status.ts`, banners de billing, `bg-white` → `bg-card`) ganhando pares `dark:`; hero de `(auth)/layout.tsx` e ícone da marca Google mantidos intactos (intencionalmente não temáticos). Build/lint/typecheck/testes verdes; smoke test do servidor sem crash. | feat |

### Ainda em aberto (não bloqueiam)

- ~~**Concorrência de reserva**~~ — concluído na Fase 7.
- ~~**Buffer entre atendimentos + granularidade de slot**~~ — concluído na Fase 8.
- ~~**Remarcação self-service**~~ — concluído na Fase 9.
- ~~**Tema escuro**~~ — concluído na Fase 10.

Todos os itens pendentes registrados ao fim da Fase 6 foram concluídos (Fases 7-10).
